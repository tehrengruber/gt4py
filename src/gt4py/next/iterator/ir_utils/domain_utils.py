        ...
        if len(shift) == 2:
            ...
            if isinstance(off, itir.CartesianOffset):
                ...
                if isinstance(connectivity_type, common.Dimension):
                    ...
                    # unstructured shift
                    assert (
                                   isinstance(val, itir.OffsetLiteral) and isinstance(val.value,
                                                                                      int)
                           ) or val in [
                    ...
                    if symbolic_domain_sizes is not None and new_dim.value in symbolic_domain_sizes:
                        ...
                        start, stop = (int(literal.value) for literal in
                                       range_exprs)  # type: ignore[attr-defined]  # mypy does not understand assert above

                        nb_index: slice | int
                        if val in [trace_shifts.Sentinel.ALL_NEIGHBORS,
                                   trace_shifts.Sentinel.VALUE]:
                            nb_index = slice(None)
                        else:
                        ...
                        fraction_accessed = np.unique(accessed).size / (
                                new_stop - new_start)  # type: ignore[call-overload]  # TODO(havogt): improve typing for NDArrayObject
